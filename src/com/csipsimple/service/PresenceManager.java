...
public class PresenceManager {
    ...
    private synchronized void updateRegistrations() {
        ...
        if (c != null && c.getCount() > 0) {
            try {
                //change by chw 在此处将账户号码为18612567487的账户设置为管理员，管理员有进入设置的权限
                String AdminSipUserName = ConfigInfoUtil.getAdminSipUserName(service);
                boolean isEnterSettingFlag = false ;
                boolean isLoop = true ;
                //end change by chw 在此处将账户号码为18612567487的账户设置为管理员，管理员有进入设置的权限 
                if (c.moveToFirst()) {
                    do {
                      ...
                      //change by chw 在此处将账户号码为18612567487的账户设置为管理员，管理员有进入设置的权限
                        Log.d(THIS_FILE, "您好，您的账号是: " + acc.getSipUserName());
                        //一直进行遍历所有的账户，当有管理员账户后，就不允许进行再判定了
                        if(isLoop) {
                        	if(AdminSipUserName.equals(acc.getSipUserName())) {		//将进入设置的状态更改为true
                            	isEnterSettingFlag = true ;
                            	isLoop = false ;	
                            }
                        }
                      //end change by chw 在此处将账户号码为18612567487的账户设置为管理员，管理员有进入设置的权限
                ...
                //end change by chw 在此处将账户号码为18612567487的账户设置为管理员，管理员有进入设置的权限
                if(isEnterSettingFlag) {
                	PreferencesProviderWrapper prefProviderWrapper = new PreferencesProviderWrapper(service);
                	prefProviderWrapper.setPreferenceBooleanValue("isEnterSetting", true);
                	Log.d(THIS_FILE, "您好，您是管理员,管理员账号是: " + AdminSipUserName);
                }
                //end change by chw 在此处将账户号码为18612567487的账户设置为管理员，管理员有进入设置的权限
            } catch (Exception e) {
