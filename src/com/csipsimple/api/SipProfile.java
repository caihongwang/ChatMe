...
public class SipProfile implements Parcelable {
     ...
     * 自动选择传输层的传输方式.<br/>
     * By default it uses UDP, if packet is higher than UDP limit it will switch to TCP.<br/>
     * 默认使用UDP协议传输，如果被设置啦比UDP更高的协议传输限制，他将会自动转换到TCP协议传输.<br/>
     * Take care with that , because not all sip servers support udp/tcp
     * 所以特别注意一些，因为有的sip服务器不会同时支持UDP/TCP协议
     * correclty.
     */
     ...
     * 获取sip服务器地址当acc_id 是这种格式 username@domain。
