    ...
    */
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        */
        ...
        //对底部视频 拨号 删除按钮进行设置可点击性
        callBar.setOnDialActionListener(this);
        //将视频的拨号按钮默认从SharePreference中获取,默认是关闭状态
        callBar.setVideoEnabled(prefsWrapper.getPreferenceBooleanValue(SipConfigManager.USE_VIDEO));
        //change by chw 20150914 将视频通话按钮设置为可见，可点击
//        callBar.setVideoEnabled(!prefsWrapper.getPreferenceBooleanValue(SipConfigManager.USE_VIDEO));
        //end change by chw 20150914 将视频通话按钮设置为可见，可点击
...
//            callBar.setVideoEnabled(!prefsWrapper.getPreferenceBooleanValue(SipConfigManager.USE_VIDEO));
    ...
    public void setTextDialing(boolean textMode) {
        Log.d(THIS_FILE, "切换拨号输入模式  " + textMode);
    ...
    private void placeCallWithOption(Bundle b) {
        ...
        // 清空被拨号的号码，便于下次拨打电话直接输入
        digits.getText().clear();

        //拨打电话
        if (accountToUse >= 0) {
            //如果存在可进行拨号Sip账户,就调用sipServer进行拨打电话
            try {
                service.makeCallWithOptions(toCall, accountToUse.intValue(), b);
            } catch (RemoteException e) {
                Log.e(THIS_FILE, "ISipService服务不能被调用,无法拨打电话...");
            }
